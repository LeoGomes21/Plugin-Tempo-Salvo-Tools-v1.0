...
def atualizar_valores_poligono(camada, fid):
    ...
    """
    index_perimetro = camada.fields().indexOf("Perimetro") # Obtém o índice do campo 'Perimetro'
    index_area = camada.fields().indexOf("Area") # Obtém o índice do campo 'Area'

    feature = camada.getFeature(fid) # Obtém o recurso com o ID fornecido
    # Verifica se o recurso é válido e tem uma geometria
    if feature.isValid() and feature.geometry() and not feature.geometry().isEmpty():
        perimetro = round(feature.geometry().length(), 3) # Calcula o perímetro do recurso
        area = round(feature.geometry().area(), 3) # Calcula a área do recurso
        
        # Atualiza os valores de 'Perimetro' e 'Area' do recurso
