...
class PlatoManager(QDialog, FORM_CLASS):
    ...
    def get_z_value(self, x, y):
        ...
        if identify_result.isValid():
            ...
            if results:
                ...
                return round(float(results[band_key]), 3)
        return None
    ...
    def handle_layers_added(self, layers):
        ...
        # Atualiza a exibição do polígono se houver uma camada de polígono adicionada
        ...
        if polygon_layers:
            ...
            # Conecta os sinais featuresAdded e featuresDeleted para detectar adição e remoção de feições nas camadas de polígono
            for polygon_layer in polygon_layers:
                polygon_layer.featureAdded.connect(self.display_polygon)
                polygon_layer.featureDeleted.connect(self.display_polygon)
        ...
        if selected_layer and isinstance(selected_layer, QgsVectorLayer):
            ...
            if len(features) == 1 or selected_layer.selectedFeatureCount() == 1:
                ...
                for i, vertex in enumerate(vertices):
                    ...
                    # Obtém o valor de Z a partir do raster
                    z = None
                    identify_result = raster_layer.dataProvider().identify(QgsPointXY(x, y), QgsRaster.IdentifyFormatValue)
                    ...
                    if identify_result.isValid():
                        results = identify_result.results()
                        if results:
                            # Pega o valor da primeira banda disponível
                            band_key = list(results.keys())[0]  # Pega a primeira banda encontrada
                            z = round(float(results[band_key]), 3)
                        else:
                            self.mostrar_mensagem(f"Não foi possível identificar Z nas coordenadas ({x}, {y}).", "Erro")
                    else:
                        self.mostrar_mensagem(f"Identificação inválida nas coordenadas ({x}, {y}).", "Erro")
