from qgis.core import (QgsProject, QgsVectorLayer, QgsPointXY, QgsField, QgsFeature, QgsGeometry, QgsPalLayerSettings, QgsVectorLayerSimpleLabeling, 
...
class GradeManager(QDialog, FORM_CLASS):
    ...
    def mostrar_mensagem(self, texto, tipo, duracao=2):
        """
        Exibe uma mensagem na barra de mensagens do QGIS com um determinado tipo e duração.

        Parâmetros:
        - texto (str): O texto da mensagem que será exibida na barra de mensagens.
        - tipo (str): O tipo de mensagem que pode ser "Erro" ou "Sucesso".
                      Dependendo do tipo, a mensagem será exibida com um ícone e cor diferentes.
        - duracao (int, opcional): A duração (em segundos) pela qual a mensagem será exibida. 
                                   O valor padrão é 2 segundos.

        Funcionalidades:
        - Obtém a barra de mensagens do QGIS para exibir a notificação.
        - Verifica o tipo de mensagem:
          - Se for "Erro", exibe a mensagem com o nível `Qgis.Critical` (mensagem de erro).
          - Se for "Sucesso", exibe a mensagem com o nível `Qgis.Info` (mensagem de informação).
        - Define a duração da exibição da mensagem.

        Retorno:
        - Nenhum. A função apenas exibe a mensagem na interface do QGIS.
        """

        # Obtém a barra de mensagens da interface do QGIS para exibir a notificação
        bar = iface.messageBar()

        # Verifica o tipo de mensagem e exibe a notificação correspondente
        if tipo == "Erro":
            # Exibe uma mensagem de erro com nível crítico e duração especificada
            bar.pushMessage("Erro", texto, level=Qgis.Critical, duration=duracao)
        elif tipo == "Sucesso":
            # Exibe uma mensagem de sucesso com nível informativo e duração especificada
