...
class UiManager:
    ...
    def abrir_caixa_nome_camada(self):
        """
        Exibe uma caixa de diálogo para o usuário inserir o nome de uma nova camada de linhas.

        Funcionalidades:
        - Cria e configura uma caixa de diálogo com título e um campo de texto para entrada do nome da camada.
        - Adiciona um placeholder ao campo de texto para sugerir que o nome da camada é temporário.
        - Inclui botões "OK" (Adicionar) e "Cancelar" para aceitar ou rejeitar a entrada do usuário.
        - Habilita o botão "OK" apenas se algum texto for inserido no campo de texto, evitando a criação de camadas sem nome.
        - Configura ações para os botões: o botão "OK" submete a caixa de diálogo com sucesso, enquanto o "Cancelar" rejeita qualquer mudança.
        - Se o usuário confirmar a caixa de diálogo com um nome válido, chama a função para criar uma nova camada com esse nome e atualiza a visualização do treeView.

        Atribuição no código:
        Esta função é fundamental para permitir que o usuário nomeie e crie novas camadas de linhas de forma interativa, garantindo que todas as novas camadas tenham um nome adequado e sejam adicionadas corretamente ao projeto e à interface do usuário.
        """
        ...
        okButton = QPushButton("Adicionar") # Cria botões OK e Cancelar
        cancelButton = QPushButton("Cancelar") # Cria um novo botão 'Cancelar'

        okButton.clicked.connect(dialog.accept) # Conecta o clique do botão 'Adicionar' à função 'accept' da caixa de diálogo
        cancelButton.clicked.connect(dialog.reject) # Conecta o clique do botão 'Cancelar' à função 'reject' da caixa de diálogo

        okButton.setEnabled(False) # Desativa o botão 'Adicionar' por padrão

        # Ativa o botão 'Adicionar' quando o campo de texto não está vazio
        ...
        buttonLayout.addWidget(okButton)  # Adiciona o botão 'Adicionar' ao layout do botão
        ...
        if dialog.exec_() == QDialog.Accepted and lineEdit.text().strip():
            ...
            criar_camada_linhas(self.iface, nome_camada)  # Cria uma nova camada de polígonos
            self.atualizar_treeView_lista_linha()  # Atualiza a árvore de visualização
    ...
    def criar_modelo_para_treeview(self):
        """
        Cria e configura um modelo de dados para o treeView, que é usado para listar as camadas de linhas no projeto.

        Funcionalidades:
        - Cria um objeto QStandardItemModel, que serve como o modelo de dados para a treeView.
        - Define o cabeçalho do modelo com o título "Lista de Camadas de Linhas", organizando visualmente a informação na interface.
        - Conecta o sinal 'dataChanged' do modelo ao método on_data_changed, que irá tratar mudanças nos dados do modelo para manter a sincronia com a interface.
        - Aplica este modelo criado ao treeView da interface, permitindo que o treeView mostre as camadas de linha conforme são adicionadas, removidas ou modificadas.
        - Chama o método configurar_treeview_header para ajustar as configurações do cabeçalho do treeView, garantindo que a apresentação seja adequada e clara.

        Atribuição no código:
        Este método é crucial para a inicialização e manutenção da lista de camadas no treeView, garantindo que a estrutura de dados esteja corretamente configurada e conectada à interface do usuário. Permite uma gestão visual eficaz das camadas de linhas, facilitando a interação do usuário com o projeto.
        """
        self.model = QStandardItemModel()  # Cria o modelo de dados para a TreeView
        self.model.setHorizontalHeaderLabels(["Lista de Camadas de Linhas"]) # Define o cabeçalho da TreeView
        # Conecta o sinal que indica mudanças de dados ao método apropriado
    ...
    def adicionar_camadas_ao_modelo(self):
        """
        Adiciona as camadas de linha disponíveis no projeto QGIS ao modelo do treeView.

        Funcionalidades:
        - Acessa a raiz da árvore de camadas do projeto QGIS para obter uma referência às camadas disponíveis.
        - Itera sobre todas as camadas de linha obtidas pelo método obter_camadas_de_linha, que filtra as camadas por tipo específico.
        - Para cada camada, cria um item representativo usando a função criar_item_para_camada, que configura a exibição e propriedades do item no treeView.
        - Adiciona cada item criado ao modelo de dados do treeView, permitindo que estas camadas sejam visualizadas e gerenciadas pela interface do usuário.
        - Conecta o sinal de mudança de visibilidade de cada camada ao modelo para garantir a atualização em tempo real da interface quando a visibilidade da camada é alterada no projeto.

        Atribuição no código:
        Essencial para a funcionalidade de visualização e gerenciamento das camadas de linha dentro do treeView. Facilita a interação do usuário com as camadas, permitindo operações como seleção, visibilidade e edição de forma intuitiva e eficiente. Assegura que todas as camadas relevantes estejam acessíveis através da interface do usuário, refletindo o estado atual do projeto QGIS.
        """
        ...
        for layer in self.obter_camadas_de_linha():
            # Cria um item para a camada e o adiciona ao modelo da TreeView
            item = self.criar_item_para_camada(layer)
            self.model.appendRow(item) # Adiciona o item ao modelo
            # Conecta o sinal de mudança de visibilidade da camada
            self.conectar_sinal_de_visibilidade(layer)
    ...
    def aplicar_cor_linha(self, layer, nova_cor):
        """
        Aplica uma nova cor à linha de uma camada especificada no projeto QGIS, atualizando tanto a representação visual da camada no mapa quanto na interface do usuário.

        Parâmetros:
        - layer (QgsMapLayer): A camada cuja cor da linha será alterada.
        - nova_cor (QColor): A nova cor que será aplicada às linhas da camada.

        Funcionalidades:
        - Obtém o renderizador atual da camada, que controla como as feições da camada são visualizadas no mapa.
        - Verifica se o renderizador é do tipo QgsSingleSymbolRenderer, que usa um único símbolo para todas as feições. Se for, altera a cor desse símbolo para a nova cor especificada.
        - Se o renderizador for do tipo QgsCategorizedSymbolRenderer, que usa símbolos diferentes para diferentes categorias de feições, altera a cor de todos os símbolos de categoria para a nova cor.
        - Atualiza a visualização da camada no mapa para refletir a nova cor, e reaplica o renderizador à camada para garantir que a mudança seja efetiva.
        - Atualiza a simbologia da camada na árvore de camadas do QGIS e a lista de camadas de linhas no diálogo do usuário para garantir que a interface reflita as mudanças.

        Atribuição no código:
        Essencial para a personalização da apresentação visual das camadas no projeto, permitindo ajustes dinâmicos na aparência das camadas de acordo com as preferências do usuário ou requisitos específicos do projeto. Melhora a flexibilidade e a interatividade do sistema, facilitando o gerenciamento visual das camadas.
        """
        renderer = layer.renderer()
        # Verifica se o renderizador é do tipo QgsSingleSymbolRenderer
        if isinstance(renderer, QgsSingleSymbolRenderer):
            symbol = renderer.symbol() # Obtém o símbolo do renderizador
            symbol.setColor(nova_cor) # Define a nova cor para o símbolo
            layer.triggerRepaint()  # Atualiza a visualização da camada
            layer.setRenderer(renderer)  # Aplica o renderizador atualizado
        
        # Verifica se o renderizador é do tipo QgsCategorizedSymbolRenderer
        elif isinstance(renderer, QgsCategorizedSymbolRenderer):
            for cat in renderer.categories(): 
                cat.symbol().setColor(nova_cor) # Define a nova cor para o símbolo da categoria
            layer.triggerRepaint() # Atualiza a camada no mapa
            layer.setRenderer(renderer) # Define o novo renderizador

        # Atualiza a simbologia da camada na árvore de camadas do QGIS
        self.iface.layerTreeView().refreshLayerSymbology(layer.id())

        # Atualiza a exibição da lista de camadas de linhas no diálogo
        self.atualizar_treeView_lista_linha()
    ...
    def tratar_linhas(self, new_layer):
        ...
        - Inicia a edição da camada e atualiza o campo de comprimento ('Comp') para todas as feições existentes baseado em suas geometrias, e depois salva as alterações.
        ...
        # Atualiza os valores de "Comp" para todas as feições existentes, se necessário
        index_comp = fields.indexOf("Comp") # Obtém o índice do campo 'Comp'
        if index_comp != -1:
            ...
            for feature in new_layer.getFeatures():
                if feature.geometry():  # Se a feição possui geometria
                    ...
                    new_layer.changeAttributeValue(feature.id(), index_comp, comprimento) # Atualiza o atributo 'Comp'
    ...
    def atualizar_comprimento_linha(self, camada, fid):
        """
        Atualiza o valor do campo 'Comp' com o comprimento da geometria de uma feição específica, sempre que essa feição é adicionada ou modificada na camada.
        ...
        Funcionalidades:
        - Encontra o índice do campo 'Comp' que deve armazenar o comprimento da linha dentro dos atributos da feição.
        ...
        - Atualiza o valor do campo 'Comp' com o comprimento calculado para a feição especificada, garantindo que o atributo esteja sempre sincronizado com a geometria atual.
        ...
        """
        # Encontra o índice do campo 'Comp' na camada
        index_comp = camada.fields().indexOf("Comp")
        ...
        if feature.geometry():
            ...
            # Atualiza o valor do campo 'Comp' com o comprimento calculado
    ...
    def abrir_adicionar_arquivo(self):
        ...
        Funcionalidades:
        - Apresenta um diálogo de seleção de arquivo com filtros para diferentes formatos suportados pelo QGIS, incluindo DXF, KML, Shapefiles, GeoJSON e Geopackages.
        ...
        - Se a camada for válida e tiver um sistema de referência de coordenadas (SRC) definido, adiciona a camada ao projeto e a torna visível.
        ...
        Atribuição no código:
        Facilita a integração de dados geoespaciais de diversos formatos no QGIS, melhorando a capacidade do usuário de gerenciar e visualizar dados geográficos em um ambiente unificado. A função ajuda a manter a flexibilidade e a abertura do QGIS para trabalhar com uma variedade de fontes de dados, tornando-o uma ferramenta essencial para profissionais de SIG.
        """
        # Cria a string de filtro para o diálogo de seleção de arquivos
        file_filter = "All Supported Formats (*.dxf *.kml *.shp *.geojson *.gpkg);;" \
                      "DXF Files (*.dxf);;" \
                      "KML Files (*.kml);;" \
                      "Shapefiles (*.shp);;" \
...
class ExportDialogDXF(QDialog):
    ...
    def adicionar_padrao_personalizado(self, doc, linetype_name, scale_factor, feature_length, text):
        ...
        - feature_length (float): Comprimento da feição a ser usado para calcular o número de repetições do padrão.
    ...
    def adicionar_padrao_setas(self, doc, linetype_name, scale_factor, feature_length):
        ...
        - feature_length: Comprimento da feição para calcular o número de repetições do padrão.
        ...
        long_dash = 0.3 * scale_factor  # Comprimento do traço ajustado
        short_dash = 0.5 * scale_factor  # Comprimento do traço curto
    ...
    def adicionar_padrao_complexo1(self, doc, linetype_name, scale_factor, feature_length):
        ...
        - feature_length (float): Comprimento da feição para calcular o número de repetições do padrão.
        ...
        long_dash = 0.5 * scale_factor # Comprimento do traço ajustado para coincidir com o símbolo
    ...
    def adicionar_padrao_complexo2(self, doc, linetype_name, scale_factor, feature_length):
        ...
        long_dash = 0.5 * scale_factor  # Comprimento do traço ajustado
    ...
    def draw_custom_text_pattern(self, scene, x_start, y_start, x_end, y_end, scale_factor, text):
        ...
        # Comprimento total de uma repetição do padrão
    ...
    def draw_custom_complex_pattern(self, scene, x_start, y_start, x_end, y_end, scale_factor):
        ...
        # Comprimento dos traços entre os símbolos e nos extremos
        inter_symbol_dash_length = max(5, 8 * scale_factor)
        start_end_dash_length = max(5, 8 * scale_factor)

        # Comprimento disponível para símbolos e traços
        total_length = x_end - x_start - 2 * start_end_dash_length
        num_symbols = max(3, int((total_length + inter_symbol_dash_length) / (symbol_width + inter_symbol_dash_length)))
        symbol_spacing = (total_length - num_symbols * symbol_width - (num_symbols - 1) * inter_symbol_dash_length) / (num_symbols - 1) if num_symbols > 1 else 0
...
class CloneManager:
    ...
    def clone_combinar(self):
        """
        Combina a clonagem de feições com a adição e ajuste de campos específicos ("ID" e "Comp"), 
        resultando em uma camada clonada que inclui campos originais, excluindo possíveis duplicatas 
        de "ID" e "Comp", e adiciona estes dois campos com valores atualizados.

        Processo:
        1. Prepara uma nova estrutura de campos, adicionando campos "ID" e "Comp" antes dos campos originais.
        2. Exclui os campos "ID" e "Comp" da camada original se já existirem, para evitar duplicatas.
        3. Itera por cada feição na camada original, criando novas feições com geometrias idênticas.
        4. Ajusta os valores de "ID" (incremental) e "Comp" (comprimento da geometria) para cada nova feição.
        ...
        7. Aplica um tratamento adicional para os campos "ID" e "Comp" na camada clonada, se necessário.
        8. Exibe uma mensagem de sucesso ao concluir a operação.
        """

        # Inicializa a estrutura de campos para a nova camada, incluindo "ID" e "Comp"
        fields_to_add = QgsFields()

        # Adiciona primeiramente os campos "ID" e "Comp"
        fields_to_add.append(QgsField("ID", QVariant.Int))
        fields_to_add.append(QgsField("Comp", QVariant.Double))

        # Recupera os campos da camada original
        original_fields = self.layer_to_clone.fields()

        # Adiciona os campos da camada original, excluindo "ID" e "Comp" se já existirem
        for field in original_fields:
            if field.name() not in ["ID", "Comp"]:
        ...
        for index, feature in enumerate(self.layer_to_clone.getFeatures(), start=1):
            ...
            # Ajusta os valores de "ID" e "Comp" diretamente
            new_attributes[0] = index  # ID baseado no índice da iteração
            new_attributes[1] = round(feature.geometry().length(), 3) # Comp calculado a partir da geometria
        ...
        # Chama a função tratar_linhas para tratar os campos "ID" e "Comp" na camada clonada
        self.ui_manager.tratar_linhas(clone)

        if clone: # Aplica tratamento adicional se necessário
            # Chama a função para tratar "ID" e "Comp" se necessário
    ...
    def clone_excluir(self):
        ...
        adicionando apenas dois novos campos: "ID" e "Comp". "ID" é um identificador sequencial
        e "Comp" representa o comprimento da geometria da feição. Esta função é útil para simplificar
        a camada, mantendo apenas informações essenciais e a geometria.

        Processo:
        1. Prepara uma nova estrutura de campos contendo apenas "ID" e "Comp".
        ...
        5. Aplica um tratamento adicional para os campos "ID" e "Comp", se necessário.
        6. Exibe uma mensagem de sucesso ao concluir a operação.
        """

        # Inicializa os campos para a nova camada com "ID" e "Comp"
        new_fields = QgsFields()
        new_fields.append(QgsField("ID", QVariant.Int))
        new_fields.append(QgsField("Comp", QVariant.Double))

        # Lista para armazenar as novas feições com os valores de "ID" e "Comp" definidos
        new_features = []
        for index, feature in enumerate(self.layer_to_clone.getFeatures(), start=1):
            ...
            new_feature.setAttribute("Comp", round(feature.geometry().length(), 3))  # Calcula e atribui o comprimentodecimais
            new_features.append(new_feature) # Adiciona a nova feição à lista
        ...
        # Chamada para tratar os campos "ID" e "Comp" após a clonagem
        self.ui_manager.tratar_linhas(clone)
