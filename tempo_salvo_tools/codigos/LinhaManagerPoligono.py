...
class LinhaManager(QDialog, FORM_CLASS):
    ...
    def convert_lines_to_polygons(self):
        ...
        if index >= 0:  # Verifica se o índice é válido
            layer_id = self.comboBoxCamada.itemData(index)  # Obtém a ID da camada selecionada
            line_layer = QgsProject.instance().mapLayer(layer_id)  # Obtém a camada correspondente à ID
            if line_layer:
                source_crs = line_layer.crs()  # Obtém o CRS da camada de linha
                is_geographic = source_crs.isGeographic()  # Verifica se o CRS é geográfico

                # Verifica se o checkBoxSeleciona está marcado
                if self.findChild(QCheckBox, 'checkBoxSeleciona').isChecked():
                    # Se o checkbox está marcado, obtemos as feições selecionadas
                    features = line_layer.selectedFeatures()
                    if not features:  # Se não houver feições selecionadas
                        self.mostrar_mensagem("Nenhuma feição selecionada. Convertendo todas as feições.", "Aviso")
                        features = line_layer.getFeatures()  # Obtém todas as feições como fallback
                else:
                    # Caso o checkbox não esteja marcado, converte todas as feições
                    features = line_layer.getFeatures()

                # Verificação de erros: verificar se todas as linhas são fechadas
                for feature in features:  # Itera sobre as feições selecionadas ou todas as feições
                    if not feature.geometry().asPolyline()[-1] == feature.geometry().asPolyline()[0]:  # Verifica se a linha é fechada
                        self.mostrar_mensagem("Erro: A camada contém linhas abertas que não podem ser convertidas em polígonos.", "Erro")
                        return

                # Define o nome da nova camada e garante que ele seja único
                ...
                # Cria uma nova camada de polígonos
                polygon_layer = QgsVectorLayer('Polygon?crs=' + line_layer.crs().authid(), layer_name, 'memory')
                provider = polygon_layer.dataProvider()

                # Estima o total de etapas para a barra de progresso
                total_steps = sum(1 for _ in features)  # Conta o número de feições
                total_steps += 2  # Adiciona duas etapas: uma para iniciar a barra e outra para a adição de campos

                # Inicia a barra de progresso antes da adição dos campos
                progressBar, progressMessageBar = self.iniciar_progress_bar(total_steps)

                # Adiciona campos à nova camada
                ...
                # Adiciona os campos da camada de linha se o checkBoxRemover não estiver marcado
                if not self.findChild(QCheckBox, 'checkBoxRemover').isChecked():
                    new_fields += fields.toList()

                start_time = time.time()  # Marca o início da conversão

                provider.addAttributes(new_fields)  # Adiciona os novos campos à camada de polígonos
                polygon_layer.updateFields()

                # Atualiza a barra de progresso para a adição de campos
                progressBar.setValue(1)

                # Reinicia o iterador de features após contagem anterior
                features = line_layer.getFeatures() if not self.findChild(QCheckBox, 'checkBoxSeleciona').isChecked() else line_layer.selectedFeatures()

                # Converte cada linha em um polígono e adiciona os atributos
                step = 1
                ...
                for feature in features:  # Itera sobre as feições selecionadas ou todas as feições
                    ...
                    step += 1
                    # Atualiza a barra de progresso a cada incremento de etapas definidas
                    if step % step_increment == 0 or step == total_steps:
                        progressBar.setValue(step)

                end_time = time.time()  # Marca o fim da conversão
                duration = end_time - start_time  # Calcula o tempo de conversão

                # Configura os cálculos automáticos para área e perímetro
                self.setup_field_calculations(polygon_layer)

                # Conecta o sinal de alteração de geometria para atualizar os valores de área e perímetro
                polygon_layer.geometryChanged.connect(self.on_geometry_changed)

                # Verifica se a projeção foi alterada e reprojeta a nova camada, se necessário
                final_layer = self.reproject_layer_if_needed(polygon_layer, line_layer, layer_name)

                # Aplica as cores escolhidas à camada final de polígonos, se o botão foi acionado
                ...
                # Adiciona a camada final ao projeto
                QgsProject.instance().addMapLayer(final_layer)

                # Garante que final_layer é uma camada válida antes de aplicar rótulos
                ...
                self.mostrar_mensagem(f"Linhas convertidas para polígonos e camada adicionada ao projeto em {duration:.2f} segundos.", "Sucesso")

                # Verifica se o checkBoxDel está selecionado e deleta a camada original
