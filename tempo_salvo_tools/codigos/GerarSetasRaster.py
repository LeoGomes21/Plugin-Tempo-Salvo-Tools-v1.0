...
from qgis.PyQt.QtWidgets import QDialog, QCheckBox, QComboBox, QPushButton, QGraphicsView, QGraphicsScene, QGraphicsPixmapItem, QColorDialog, QRadioButton, QProgressBar
...
class SetasManager(QDialog, FORM_CLASS):
    ...
    def mostrar_mensagem(self, texto, tipo, duracao=3):
        """
        Exibe uma mensagem na barra de mensagens do QGIS, proporcionando feedback ao usuário com base nas ações realizadas.

        Parâmetros:
        - self: Referência para a instância atual da classe (implícito), utilizada para acessar os atributos e métodos da classe.
        - texto (str): O texto da mensagem a ser exibida ao usuário.
        - tipo (str): O tipo de mensagem a ser exibida. Pode ser "Erro" para mensagens de erro ou "Sucesso" para mensagens informativas.
        - duracao (int): O tempo (em segundos) durante o qual a mensagem será exibida. O valor padrão é 3 segundos.

        A função realiza as seguintes ações:
        - Acessa a barra de mensagens do QGIS.
        - Exibe uma mensagem com base no tipo especificado ("Erro" ou "Sucesso").
        - Define o ícone e a cor apropriados para o tipo de mensagem.
        - Define a duração que a mensagem ficará visível.

        Retorno:
        - Nenhum retorno explícito. A função exibe diretamente uma mensagem na barra de mensagens do QGIS.
        """
    ...
    def exibir_rotulo_inclinacao(self, layer):
        ...
        layer_settings.fieldName = """
            CASE 
                WHEN "Inclinacao" < 0 THEN '⬇' || abs(round("Inclinacao" * 100, 3)) || '%'
                WHEN "Inclinacao" > 0 THEN '⬆' || round("Inclinacao" * 100, 3) || '%'
                ELSE '→' || round("Inclinacao" * 100, 3) || '%' 
            END
