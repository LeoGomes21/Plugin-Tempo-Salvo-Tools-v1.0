...
class CurvasManager(QDialog, FORM_CLASS):
    ...
    def export_to_dxf(self):
        """
        Exporta a camada 'Curva de Níveis 3D' para um arquivo DXF, incluindo simbologia e rótulos.

        Parâmetros:
        - Nenhum parâmetro explícito é passado diretamente para esta função, mas ela acessa e manipula os seguintes atributos da classe:
          - self.escolher_local_para_salvar(): Função que permite o usuário escolher onde salvar o arquivo DXF.
          - self.mostrar_mensagem(): Função que exibe mensagens ao usuário.
          - self.iniciar_progress_bar(): Função que exibe uma barra de progresso durante o processo de exportação.
          - self.spinBoxEscala: SpinBox que contém o valor de escala para a exportação.

        Funcionalidades:
        - Verifica se a camada 'Curva de Níveis 3D' está presente e válida no projeto.
        - Permite ao usuário escolher o local para salvar o arquivo DXF.
        - Configura os parâmetros de exportação, incluindo simbologia, rótulos e coordenadas Z.
        - Exibe uma barra de progresso durante o processo de exportação.
        - Lida com o processo de exportação, atualizando a barra de progresso e exibindo mensagens de sucesso ou erro.
        """
        layer_name = "Curva de Niveis 3D"
        memory_layer = None

        # Busca a camada "Curva de Níveis 3D"
        for layer in QgsProject.instance().mapLayers().values():
            if layer.name() == layer_name:
                memory_layer = layer
                break

        if not memory_layer or not isinstance(memory_layer, QgsVectorLayer):
            return

        output_dxf_path = self.escolher_local_para_salvar(layer_name, "Arquivo DXF (*.dxf)")

        if not output_dxf_path:
            self.mostrar_mensagem("Operação cancelada pelo usuário.", "Erro")
            return

        # Verifica se a camada tem coordenadas Z para exportar como 3D
        has_z = QgsWkbTypes.hasZ(memory_layer.wkbType())

        # Obtém o SRC da camada
        src_crs = memory_layer.crs().authid()

        # Verifica se os rótulos estão ativados
        labels_enabled = memory_layer.labelsEnabled()

        # Configura os parâmetros de exportação
        params = {
            'LAYERS': [memory_layer.id()],
            'FILE_ENCODING': 'CP1256',
            'SYMBOLOGY_MODE': 2,  # 2 para usar a simbologia de camada de símbolos
            'OUTPUT': output_dxf_path,
            'EXPORT_LABELS': True,  # Exportar rótulos como MTEXT
            'USE_TITLE_AS_LAYER_NAME': True,
            'EXTENT': memory_layer.extent(),  # Usar a extensão atual da camada
            'INCLUDE_SYMBOLOGY': True,
            'SCALE': self.spinBoxEscala.value(),
            'EXPORT_LINES_WITH_ZERO_WIDTH': True,  # Exportar linhas com largura zero
            'CRS': src_crs,  # Usa o SRC da camada
            'FORCE_2D': not has_z,  # Se a camada não tiver Z, força saída 2D
            'EXPORT_MTEXT': True,  # Exportar rótulos como MTEXT
            'WITH_Z': has_z,  # Exportar como 3D se houver coordenadas Z
        }

        # Inicia a barra de progresso
        progressBar, progressMessageBar = self.iniciar_progress_bar(memory_layer)

        # Medir o tempo de início
        start_time = time.time()

        # Executa o algoritmo de exportação com atualização da barra de progresso
        try:
            for i in range(memory_layer.featureCount()):
                progressBar.setValue(i + 1)  # Atualiza a barra de progresso

                # Simula um processamento por feição (para fins de exemplo, você pode remover ou ajustar isso)
                time.sleep(0.01)  # Simula o tempo de processamento por feição

            result = processing.run("native:dxfexport", params)

            # Calcula o tempo total de exportação
            elapsed_time = time.time() - start_time

            # Exibe mensagem de sucesso com tempo de execução e opção de abrir a pasta ou o arquivo DXF
            self.mostrar_mensagem(
                f"Exportação para DXF concluída com sucesso em {elapsed_time:.2f} segundos!",
                tipo="Sucesso",
                caminho_pasta=os.path.dirname(output_dxf_path),
                caminho_arquivo=output_dxf_path
            )

        except Exception as e:
            # Exibe mensagem de erro
            self.mostrar_mensagem(f"Erro ao exportar para DXF: {e}", tipo="Erro")
        finally:
