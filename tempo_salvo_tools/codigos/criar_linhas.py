from qgis.core import (
    ...
    QgsVectorLayerSimpleLabeling, QgsUnitTypes, QgsDistanceArea
)
from PyQt5.QtGui import QFont, QColor
from PyQt5.QtCore import QVariant 

def criar_camada_linhas(iface, nome_camada=None):
    """Cria e configura uma nova camada de linhas temporária, adicionando-a ao painel de camadas do QGIS."""

    crs_projeto = obter_crs_projeto_atual()
    nome_camada = gerar_nome_camada(nome_camada)

    camada_linhas = criar_camada_vectorial(nome_camada, crs_projeto)
    adicionar_campos(camada_linhas)
    configurar_campo_oculto(camada_linhas, "Comp")
...
def adicionar_campos(camada):
    """Adiciona campos ID e Comp à camada, com restrições no campo ID."""
    id_field = QgsField("ID", QVariant.Int)
    comp_field = QgsField("Comp", QVariant.Double)
